...
def reading(rfc, c):
    ...
    if rfc == "RFC 6605":
        ...
        return [
            ...
            f'Reverse DNS shows operators switching in batches, not diffusing, and the trigger cannot be identified. New signings '
            f'ignored the Knot 2.1.0 and PowerDNS 4.0.0 defaults on their upstream dates ({ar["2.1.0"]["share_before_pct"]}% -> '
            f'{ar["2.1.0"]["share_after_pct"]}% and {ar["4.0.0"]["share_before_pct"]}% -> {ar["4.0.0"]["share_after_pct"]}%), then stepped '
            f'{len(AT["families"]["ECDSA"]["steps"])} times: {", ".join(x["quarter"] for x in AT["families"]["ECDSA"]["steps"])}. The first is '
            f'{ST["detail"]["signings"]} signings of which {max(ST["detail"]["by_month"].values())} fall in one month, all but '
            f'{sum(v for k, v in ST["detail"]["by_rir"].items() if k != "apnic")} in APNIC, one parent block '
            f'{ST["detail"]["largest_block_share"]*100:.0f}% of it. Each step has a different OS release behind it and '
            f'{CH["within_3m"]*100:.0f}% of all months follow some OS release within 3 months anyway.',
        ]
    if rfc == "RFC 5702":
...
def main() -> int:
    ...
    text(s, Inches(0.62), Inches(1.3), Inches(12.1), Inches(5.4), [
        ...
        f'blocks. An OS release precedes every step -- a different one each time -- while {CH["within_3m"]*100:.0f}% of all months follow some OS '
        f'release within 3 months, and two of the five RSA/SHA-256 steps have no OS release in the year before them at all.',
        "An update can only change what a zone signed after it gets, never an existing zone, so this is where a default would act. That the steps "
        "exist is evidence operators moved in batches once the defaults existed upstream; which delivery path reached any of them is not identifiable "
    ...
    text(s, Inches(0.62), Inches(1.3), Inches(12.1), Inches(5.4), [
        ...
        "OS packages cover Debian 9-13 and Ubuntu LTS 16.04-24.04 by version, plus RHEL majors by date. They are one delivery path among many: "
        "RHEL base carries only BIND (Knot and PowerDNS need EPEL) and had no ECDSA-default signer until RHEL 9 in 2022-05, and FreeBSD ports, "
        "Alpine, containers and source builds are not represented at all. Package dates bound when a default became reachable, not what anyone ran.",
