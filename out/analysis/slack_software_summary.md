...
• *Operators switch in sudden batches, and I can't identify the trigger.* On their upstream dates the 2016 Knot/PowerDNS ECDSA defaults left no trace in reverse DNS (new signings <1% ECDSA before and after). New signings then stepped four times: 2017Q3, 2019Q1, 2019Q4, 2020Q3. The first is 157 signings with 155 of them in September 2017, all but two in APNIC, one parent block 40% of it. An OS release precedes every step but a different one each time (Debian 9, Ubuntu 18.04, RHEL 8/Debian 10, Ubuntu 20.04), 31% of all months follow some OS release within 3 months anyway, and two of the five RSA/SHA-256 steps have no OS release in the year before them. I had attributed the first step to Debian 9 and that does not hold — it was selection on my part. What stands: an update changes only what a zone signs *after* it, never an existing zone, so new signings are where a default acts, and operators clearly moved in batches once the defaults existed upstream.
• *NSEC3 iterations (RFC 9276) is the one vendor-triggered case.* Names with ≥100 iterations collapsed in Oct–Nov 2021 in five of the six TLDs that had any (.nu 12,145 → 110; only .gov, with 38 names, did not move), 10 months BEFORE the RFC, 2–3 months after the PowerDNS 4.5.0 and Unbound 1.13.2 caps and CVE-2021-40083 (Knot Resolver assertion failure on too many iterations). All 3 spikes are within 3 months of a cap vs a 14% chance rate. Both sides capped it — PowerDNS Auth is a signer, Unbound a validator — and the RFC codified what the vendors had already forced.
...
• Which software signs a zone is never visible; "nearest release" is timing evidence, and registry/registrar platforms are closed. The OS package table is Debian/Ubuntu/RHEL only — FreeBSD, Alpine, containers and source builds are absent, and RHEL base ships no Knot or PowerDNS at all (EPEL only), with no ECDSA-default BIND until RHEL 9 in 2022.
