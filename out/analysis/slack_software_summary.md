...
• Added the first Ubuntu LTS and Debian release that shipped each version (Launchpad + Debian tracker), since most operators get a new default through apt, not from upstream.
...
• *Auto-update:* in reverse DNS the 2016 Knot/PowerDNS defaults left no trace (new signings <1% ECDSA before and after). The share stepped from 23% to 39% around BIND 9.16.0, which Ubuntu 20.04 and Debian 11 carried. Reverse operators run BIND, and only BIND's defaults reached them.
...
• Which software signs a zone is never visible; "nearest release" is timing evidence, and registry/registrar platforms are closed.
